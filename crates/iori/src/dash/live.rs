...
use dash_mpd::{AdaptationSet, Period, MPD, Representation};
use tokio::sync::mpsc;
use url::Url;
...
use crate::{
    ...
    fetch::fetch_segment as fetch_segment_global, // Renamed to avoid conflict
    util::http::HttpClient,
    InitialSegment, SegmentType, StreamingSource, // Added StreamingSegment, InitialSegment, SegmentType
};

use tokio::sync::Mutex; // Added for Arc<Mutex<>>

#[derive(Debug)] // Removed Clone, Clock will be wrapped in Arc<Mutex<>>
pub struct Clock {
    offset: TimeDelta,
    client: HttpClient, 
}
...
impl Clock {
    ...
    async fn sync(&mut self, mpd: &MPD) -> IoriResult<()> {
        sync_time(mpd, self, &self.client).await
    }
}
...
// Helper to merge BaseURLs as defined in DASH spec
fn merge_baseurls(current_base: &Url, new_relative: &str) -> IoriResult<Url> {
    ...
    if is_absolute_url(new_relative) {
        Url::parse(new_relative).map_err(IoriError::UrlParsing)
    } else {
        let mut merged = current_base.join(new_relative).map_err(IoriError::UrlParsing)?;
        if merged.query().is_none() {
            merged.set_query(current_base.query());
        }
        Ok(merged)
    }
}
...
pub struct LiveDashSource {
    client: HttpClient, // Read-only after creation
    mpd_url: Url,       // Read-only after creation
    key: Option<Arc<IoriKey>>, // Read-only after creation
    ...
    minimum_update_period: Arc<Mutex<Option<TimeDelta>>>,
    ...
    sequence_counter: Arc<AtomicU64>, // Already thread-safe
    shaka_packager_command: Option<PathBuf>, // Read-only
    representation_selector: RepresentationSelector, // Read-only

    // Internal state for comparing segments if update_segments were to diff.
    // This is better managed locally by the task that calls update_segments repeatedly.
    // For this refactor, removing `current_segments` from LiveDashSource struct.
    // current_segments: Vec<DashSegment>, 
}
...
impl LiveDashSource {
    pub fn new(
        ...
        let clock = Arc::new(Mutex::new(Clock::new(clock_client)));
    ...
    fn get_base_url_for_representation(
        &self, 
        mpd_guard: &MPD, // Pass the locked MPD data
        period: &Period, 
        adapt_set: &AdaptationSet, 
        rep: &Representation
    ) -> IoriResult<Url> {
        let mut current_url = self.mpd_url.clone(); // Start with original MPD URL as ultimate base

        // MPD level BaseURLs
        for base_obj in &mpd_guard.base_url {
        ...
        for base_obj in &adapt_set.BaseURL {
            current_url = merge_baseurls(&current_url, &base_obj.base)?;
        }
        // Representation level BaseURLs
        for base_obj in &rep.BaseURL {
            current_url = merge_baseurls(&current_url, &base_obj.base)?;
        }
        Ok(current_url)
    }
    ...
    async fn update_segments(&self) -> IoriResult<Option<Vec<DashSegment>>> {
        ...
        let clock_guard = self.clock.lock().await; // Clock is read for now()
        ...
        let mpd = match mpd_guard.as_ref() {
            ...
            None => {
                ...
                tracing::warn!("update_segments called but MPD data is None. Returning no segments.");
                return Ok(None);
            }
        };
        ...
        let period = mpd
            .periods
            .iter()
            .find(|p| current_active_period_id_opt.is_none() || p.id.as_ref() == current_active_period_id_opt.as_ref())
        ...
        let adaptation_set = period
            ...
            .find(|adapt| {
                ...
                content_type.starts_with("video") || mime_type.starts_with("video") || content_type.starts_with("audio") || mime_type.starts_with("audio")
            })
            .ok_or(IoriError::NoAdaptationSetFound("video or audio".to_string()))?;
        ...
        let representation_base_url = self.get_base_url_for_representation(&mpd, period, adaptation_set, &representation)?;
        ...
        if let Some(segment_template) = representation.SegmentTemplate.as_ref().or(adaptation_set.SegmentTemplate.as_ref()) {
            let initialization_from_template = if let Some(init_template_str) = &segment_template.initialization {
                let init_vars = Template::new(); 
                init_vars.insert(Template::REPRESENTATION_ID, representation.id.clone().unwrap_or_default());
                init_vars.insert(Template::BANDWIDTH, representation.bandwidth.unwrap_or(0).to_string());
            ...
            if let Some(timeline) = &segment_template.SegmentTimeline {
                tracing::debug!("Processing SegmentTimeline for Rep ID: {:?}", representation.id);
                let timescale = segment_template.timescale.unwrap_or(1);
                let mut current_presentation_time_pts = timeline.segments.get(0).and_then(|s| s.t).unwrap_or(0);
                let mut current_segment_number = segment_template.startNumber.unwrap_or(1);
                
                let availability_start_time_mpd = mpd.availabilityStartTime.unwrap_or_else(|| Utc::now() - TimeDelta::days(7));
                let period_start_std_duration = period.start.unwrap_or(StdDuration::ZERO);
                let period_start_offset_from_availability = TimeDelta::from_std(period_start_std_duration)
                    .map_err(|e| IoriError::Generic(format!("Invalid period start duration: {}", e)))?;
                let absolute_period_start_time = availability_start_time_mpd + period_start_offset_from_availability;
                
                let suggested_delay_std = mpd.suggestedPresentationDelay.unwrap_or(StdDuration::from_secs(0));
                let time_shift_buffer_depth_std = mpd.timeShiftBufferDepth.unwrap_or_else(|| StdDuration::from_secs(3600 * 24 * 7));

                let live_edge_presentation_time = now_synced - TimeDelta::from_std(suggested_delay_std).map_err(|e| IoriError::Generic(e.to_string()))?;
                let earliest_available_segment_start_time = live_edge_presentation_time - TimeDelta::from_std(time_shift_buffer_depth_std).map_err(|e| IoriError::Generic(e.to_string()))?;

                for s_element in &timeline.segments {
                    if let Some(t) = s_element.t { current_presentation_time_pts = t; }
                    let duration_pts = s_element.d;
                    let repeat_count = s_element.r.unwrap_or(0);

                    for _ in 0..=repeat_count {
                        let segment_abs_start_time = absolute_period_start_time + TimeDelta::duration_seconds_f64(current_presentation_time_pts as f64 / timescale as f64);
                        let segment_duration_sec = duration_pts as f64 / timescale as f64;
                        let segment_abs_end_time = segment_abs_start_time + TimeDelta::duration_seconds_f64(segment_duration_sec);
                        
                        if segment_abs_end_time <= earliest_available_segment_start_time && mpd.timeShiftBufferDepth.is_some() {
                            current_presentation_time_pts += duration_pts; current_segment_number += 1; continue;
                        }
                        if segment_abs_start_time >= live_edge_presentation_time + TimeDelta::seconds(10) {
                            current_presentation_time_pts += duration_pts; current_segment_number += 1; continue; 
                        }

                        let mut template_vars = Template::new();
                        template_vars.insert(Template::REPRESENTATION_ID, representation.id.clone().unwrap_or_default());
                        template_vars.insert(Template::NUMBER, current_segment_number.to_string());
                        template_vars.insert(Template::BANDWIDTH, representation.bandwidth.unwrap_or(0).to_string());
                        template_vars.insert(Template::TIME, current_presentation_time_pts.to_string());

                        let media_url_template = segment_template.media.as_ref().ok_or_else(|| IoriError::MpdParsing("SegmentTemplate media attribute missing for SegmentTimeline".into()))?;
                        let relative_url = template_vars.resolve(media_url_template);
                        let segment_url = representation_base_url.join(&relative_url)?;
                        let segment_filename = relative_url.split('/').last().unwrap_or(&format!("seg_{}.m4s", current_segment_number)).to_string();
                        
                        new_segments.push(DashSegment {
                            url: segment_url, filename: segment_filename,
                            r#type: SegmentType::from_mime_type(representation.mimeType.as_deref().or(adaptation_set.mimeType.as_deref())),
                            initial_segment: initialization_from_template.as_ref().map_or(InitialSegment::None, |_| InitialSegment::None), 
                            key: self.key.clone(), byte_range: None, 
                            sequence: self.sequence_counter.fetch_add(1, Ordering::Relaxed),
                        });
                        
                        current_presentation_time_pts += duration_pts; current_segment_number += 1;
                        if new_segments.len() >= 1000 { break; }
                    }
                    if new_segments.len() >= 1000 { break; }
                }
            } else if segment_template.media.is_some() { // SegmentTemplate @duration case
                let timescale = segment_template.timescale.unwrap_or(1);
                let duration_pts = segment_template.duration.ok_or(IoriError::MpdParsing("SegmentTemplate missing duration for @duration case".into()))?;
                let duration_sec = duration_pts as f64 / timescale as f64;
                if duration_sec <= 0.0 { return Err(IoriError::MpdParsing("SegmentTemplate duration must be positive".into())); }
                ...
                let mut current_segment_number = if earliest_available_segment_start_time > absolute_period_start_time {
                    ((earliest_available_segment_start_time - absolute_period_start_time).num_milliseconds() as f64 / 1000.0 / duration_sec).floor() as u64 + start_number
                } else { start_number }.max(start_number);

                loop {
                    let segment_rel_start_pts = (current_segment_number - start_number) * duration_pts;
                    let segment_presentation_time = absolute_period_start_time + TimeDelta::duration_seconds_f64(segment_rel_start_pts as f64 / timescale as f64);

                    if segment_presentation_time > live_edge_presentation_time + TimeDelta::seconds(10) { break; }
                    if segment_presentation_time < earliest_available_segment_start_time && mpd.timeShiftBufferDepth.is_some() {
                         current_segment_number += 1; continue;
                    }
                    ...
                    let relative_url = template_vars.resolve(media_url_template);
                    let segment_url = representation_base_url.join(&relative_url)?;
                    let segment_filename = relative_url.split('/').last().unwrap_or(&format!("{}_{}.m4s", representation.id.as_deref().unwrap_or("s"), current_segment_number)).to_string();
                    ...
                    if new_segments.len() >= 1000 { break; }
                    if let Some(pds) = period.duration { if TimeDelta::from_std(pds).map_err(|e|IoriError::Generic(e.to_string()))? < (segment_presentation_time - absolute_period_start_time) { break; }}
                }
            } else { return Err(IoriError::MpdParsing("SegmentTemplate present but lacks media URL or SegmentTimeline.".into())); }
        } else if let Some(segment_list) = representation.SegmentList.as_ref().or(adaptation_set.SegmentList.as_ref()) {
            tracing::debug!("Processing SegmentList for Rep ID: {:?}", representation.id);
            let list_timescale = segment_list.timescale.unwrap_or(1);
            let list_initialization_resolved_url = if let Some(init_el) = &segment_list.Initialization {
                if let Some(s_url) = &init_el.sourceURL { Some(representation_base_url.join(s_url)?) } else { None }
            } else { None };
            ...
            for (idx, segment_url_el) in segment_list.SegmentURL.iter().enumerate() {
                let media_uri = segment_url_el.media.as_ref().ok_or_else(|| IoriError::MpdParsing(format!("SegmentURL @index {} missing @media", idx)))?;
                let segment_absolute_url = representation_base_url.join(media_uri)?;
                let segment_filename = media_uri.split(|c| c == '/' || c == '?').last().unwrap_or(&format!("sl_{}_{}.m4s", representation.id.as_deref().unwrap_or("r"), idx)).to_string();

                if let Some(list_segment_duration_pts) = segment_list.duration {
                    let seg_dur_pts_u64 = list_segment_duration_pts;
                    let seg_start_abs = absolute_period_start_time + TimeDelta::duration_seconds_f64(current_presentation_time_pts as f64 / list_timescale as f64);
                    let seg_dur_sec = seg_dur_pts_u64 as f64 / list_timescale as f64;
                    let seg_end_abs = seg_start_abs + TimeDelta::duration_seconds_f64(seg_dur_sec);

                    if seg_end_abs <= earliest_available_segment_start_time && mpd.timeShiftBufferDepth.is_some() {
                        current_presentation_time_pts += seg_dur_pts_u64; continue;
                    }
                    if seg_start_abs >= live_edge_presentation_time + TimeDelta::seconds(10) && mpd.mpdtype.as_deref() == Some("dynamic") {
                         current_presentation_time_pts += seg_dur_pts_u64; continue;
                    }
                }
                
                new_segments.push(DashSegment {
                    url: segment_absolute_url, filename: segment_filename,
                    r#type: SegmentType::from_mime_type(representation.mimeType.as_deref().or(adaptation_set.mimeType.as_deref())),
                    initial_segment: list_initialization_resolved_url.as_ref().map_or(InitialSegment::None, |_| InitialSegment::None), 
                    key: self.key.clone(), byte_range: segment_url_el.mediaRange.clone(),
                    sequence: self.sequence_counter.fetch_add(1, Ordering::Relaxed),
                });
                
                if let Some(list_dur_pts) = segment_list.duration { current_presentation_time_pts += list_dur_pts; }
                if new_segments.len() >= 2000 { break; }
            }
        } else { return Err(IoriError::MpdParsing("No SegmentTemplate or SegmentList found for selected representation.".into())); }
...
async fn live_updater_task(
    source: LiveDashSource, // Takes ownership of a LiveDashSource instance (or relevant Arcs)
    sender: mpsc::UnboundedSender<IoriResult<Vec<DashSegment>>>,
    initial_mpd_type: Option<String>, // To know if it starts as dynamic
    initial_min_update_period: Option<TimeDelta>,
) {
    let mut current_mpd_type = initial_mpd_type;
    let mut min_update_period_opt = initial_min_update_period;
    let mut last_sent_segments_snapshot: Option<Vec<DashSegment>> = None; // To compare and send only new lists

    loop {
        let sleep_duration = match min_update_period_opt {
            Some(p) if p > TimeDelta::zero() => p.to_std().unwrap_or(StdDuration::from_secs(60)), // Ensure positive
            _ => { // Default if not present or zero, or if MPD type is static
                if current_mpd_type.as_deref() != Some("dynamic") {
                    tracing::info!("MPD is static or no minimumUpdatePeriod, updater task finishing.");
                    break; 
                }
        ...
        // 1. Fetch new MPD
        tracing::info!("Updater task: Fetching new MPD from {}", source.mpd_url);
        ...
        current_mpd_type = new_mpd.mpdtype.clone();
        ...
        { // Scoped lock for shared data
            ...
            if let Err(e) = clock_guard.sync(&new_mpd).await { // Sync clock with new MPD
            ...
            *last_mpd_update_guard = Some(clock_guard.now());
            min_update_period_opt = new_mpd.minimumUpdatePeriod.map(|p| TimeDelta::from_std(p).unwrap_or_else(|_| TimeDelta::zero()));
        ...
        match source.update_segments().await {
            Ok(Some(segments)) => {
                if !segments.is_empty() {
                    // Simple diff: only send if the list is different from the last sent one.
                    // More sophisticated diffing could be done (e.g. by segment URL or sequence number).
                    let send_update = match &last_sent_segments_snapshot {
                        Some(last_sent) => *last_sent != segments,
                        None => true, // First time sending segments after initial
                    };

                    if send_update {
                        tracing::info!("Found {} new/updated segments. Sending update.", segments.len());
                        if sender.send(Ok(segments.clone())).is_err() {
                            tracing::warn!("Receiver dropped, updater task finishing.");
                            break; // Exit loop if channel closed
                        }
                        last_sent_segments_snapshot = Some(segments);
                    } else {
            ...
            Ok(None) => {
                tracing::debug!("MPD updated, but update_segments returned None (no change or no segments).");
            }
            Err(e) => {
                tracing::error!("Error updating segments after MPD refresh: {}", e);
                if sender.send(Err(e)).is_err() { // Propagate error
        ...
        if new_mpd.mpdtype.as_deref() == Some("static") {
            tracing::info!("MPD type changed to 'static', assuming live stream ended. Updater task finishing.");
...
impl StreamingSource for LiveDashSource {
    ...
    async fn fetch_info(
        ...
        // --- Initial MPD Fetch and Segment List ---
        // This part runs once to provide the first set of segments quickly.
        let initial_mpd_text = match self.client.get(self.mpd_url.clone()).send().await {
            Ok(resp) => resp.text().await.map_err(|e| IoriError::Network(format!("initial MPD text: {}", e)))?,
            Err(e) => return Err(IoriError::Network(format!("initial MPD fetch: {}", e))),
        };
        
        let initial_mpd = dash_mpd::parse(&initial_mpd_text).map_err(IoriError::MpdParsing)?;

        let initial_mpd_type = initial_mpd.mpdtype.clone();
        let initial_min_update_period = initial_mpd.minimumUpdatePeriod.map(|p| TimeDelta::from_std(p).unwrap_or_default());
        ...
        // Lock and update shared state for the first time
        {
            ...
            clock_guard.sync(&initial_mpd).await?; // Initial clock sync
        ...
        match self.update_segments().await {
            Ok(Some(segments)) => {
                if sender.send(Ok(segments)).is_err() {
                    // Receiver likely dropped immediately, probably not an error if app closes early.
                    tracing_warn!("Failed to send initial segments, receiver dropped.");
                    return Ok(receiver); // Still return receiver, it will just show channel closed.
                }
            }
            Ok(None) => { // No segments available initially
                if sender.send(Ok(vec![])).is_err() {
                     tracing_warn!("Failed to send initial empty segment list, receiver dropped.");
                     return Ok(receiver);
                }
            }
            Err(e) => {
                // Send error for initial segment fetch
                _ = sender.send(Err(e)); 
                return Ok(receiver); // Return receiver so caller can get the error.
            }
        }
        ...
        if initial_mpd_type.as_deref() == Some("dynamic") && initial_min_update_period.is_some() {
            ...
            tokio::spawn(live_updater_task(task_source, task_sender, initial_mpd_type, initial_min_update_period));
        } else {
            tracing::info!("MPD is not dynamic or lacks minimumUpdatePeriod; no background updater task will be spawned.");
...
pub async fn sync_time(mpd: &MPD, clock: &mut Clock, client: &HttpClient) -> IoriResult<()> {
    ...
    for timing in &mpd.UTCTiming {
        tracing::debug!(scheme = %timing.schemeIdUri, value = %timing.value, "Attempting to sync time with scheme");
                            ...
                            if response.status().is_success() {
                                match response.text().await {
                                    ...
                                    Err(e) => {
                                        ...
                                        last_error = Some(IoriError::Network(e.to_string()));
                                    }
                                }
                            } else {
                                tracing::warn!(url, status = %response.status(), "HTTP request for xsdate/iso8601 failed");
                                last_error = Some(IoriError::Network(format!(
                                    "HTTP status {}",
                                    response.status()
                                )));
                            }
                        ...
                        Ok(response) => {
                            if response.status().is_success() {
                                    ...
                                    response.headers().get(reqwest::header::DATE)
                                {
                                    ...
                                    tracing::warn!(url, "Missing Date header in HTTP HEAD response");
                                    last_error = Some(IoriError::DateTimeParsing(
