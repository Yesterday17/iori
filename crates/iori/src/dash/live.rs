...
impl LiveDashSource {
    ...
    fn get_base_url_for_representation(
        ...
        for base_obj in &adaptation_set.BaseURL {
            current_url = merge_baseurls(&current_url, &base_obj.base)?;
        }
    ...
    async fn update_segments(&self) -> IoriResult<Option<Vec<DashSegment>>> {
        ...
        if !period.adaptations.iter().any(|adapt| {
            ...
            return Err(IoriError::NoAdaptationSetFound);
        };

        for (adaptation_set_index, adaptation_set) in period.adaptations.iter().enumerate() {
            ...
            let representation_base_url = self.get_base_url_for_representation(
                &mpd,
                period,
                adaptation_set,
                &representation,
                ...
                .or(adaptation_set.SegmentTemplate.as_ref())
            {
                ...
                if let Some(timeline) = &segment_template.SegmentTimeline {
                    ...
                    let period_start_offset_from_availability =
                        TimeDelta::from_std(period_start_std_duration).inspect_err(|e| {
                            tracing::error!("Invalid period start duration: {e}")
                        })?;
                    ...
                    let earliest_available_segment_start_time = live_edge_presentation_time
                        - TimeDelta::from_std(time_shift_buffer_depth_std)?;
                    ...
                    for s_element in &timeline.segments {
                        ...
                        for _ in 0..=repeat_count {
                            ...
                            template_vars
                                .insert(Template::NUMBER, current_segment_number.to_string());
                            template_vars.insert(
                                Template::BANDWIDTH,
                                representation.bandwidth.unwrap_or(0).to_string(),
                            );
